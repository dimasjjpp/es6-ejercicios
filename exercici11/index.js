...
const likedVolumes = []
const likedVolumesCounter = []

for (const user of users) {
  for (const sound in user.favoritesSounds) {
    if (user.favoritesSounds[sound].format === 'ogg') {
      likedVolumes.push(sound)
      if (likedVolumes[sound]) {
        likedVolumesCounter[sound]++
      } else {
        likedVolumesCounter[sound] = 1
      }
    }
  }
}
console.log(likedVolumes)
console.log(likedVolumesCounter)
